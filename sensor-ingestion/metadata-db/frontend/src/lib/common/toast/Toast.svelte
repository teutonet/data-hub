...
<div
	transition:slide
	class="toast show bg-{color}-600 m-2 rounded-sm border-2 text-white border-{color}-600"
	role="alert"
	aria-live="assertive"
	aria-atomic="true"
>
	<div class="flex p-2">
		...
		<button type="button" class="btn-close align-self-start shrink-0" onclick={close}>
			<CloseIcon />
			<span class="sr-only">{$_('shared.action.close')}</span>
		</button>
	</div>
	{#if toast.detail}
		<div class="text-newlines rounded-sm bg-white p-2 text-black">
