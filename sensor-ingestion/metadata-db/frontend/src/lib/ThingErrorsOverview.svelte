...
<Card class="max-w-fit">
	...
	{#if things.length === 0}
		<p class="m-4 pt-4 text-black italic dark:text-slate-300">
	...
	{:else}
		<ul class="my-1 space-y-3">
			{#each things as thing (thing.id)}
				<li>
					<a
						href={projectUrl(project, 'sensor', encodeURI(thing.id))}
					...
					>
						...
						>
						<span class="ms-3 flex w-full font-normal whitespace-pre-line">{thing.latestError}</span
						>
